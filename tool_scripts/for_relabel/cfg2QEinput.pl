...
#
#die;
my @oldin = `find ./ -type f -name "*.in" -exec readlink -f {} \\;|sort`;
for my $i (@oldin){`rm -f $i`;}

#you may use relabel script to increase more labelled folders if needed
my @labelled_dir = `find ./ -type d -name "labelled" -exec readlink -f {} \\;|sort`;
...
for my $cfg (@pathOfAllcfgs){
    ...
    #print "$qeinput\n";
 #   `echo '$qeinput' > $dirname/$basename.in`;
    print "** Current qe input file number: $counter\n";
    last if($counter == $maxDFTfiles);
}
